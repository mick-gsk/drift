...
from drift_engine.signal_mapping import _ABBREV_TO_SIGNAL, signal_abbrev
...
def _select_priority_findings_from_list(
    ...
    *,
    ...
    prioritized = sorted(
        deduped,
        key=lambda f: (
            _composite_sort_key(f, file_histories=file_histories)
            + (
                -float(f.score),
                f.signal_type,
...
def build_first_run_summary(
    ...
    *,
    ...
    if is_german:
        ...
        default_next_step = (
            "Nutze drift fix-plan --repo . --max-tasks 5 fuer konkrete Reparaturschritte."
        )
    else:
        headline = (
