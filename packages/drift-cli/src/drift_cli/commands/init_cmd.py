...
def _run_interactive_profile_setup(profile: str, interactive: bool, json_output: bool) -> str:
    ...
    if not interactive or json_output:
        return profile
    from drift_cli.commands.setup import (
        _ask_ai_usage,
        _ask_project_type,
        _ask_strictness,
        _derive_profile,
    ...
    for p in plan:
        ...
        if _write_if_absent(target, str(p["content"]), str(p["path"]), root=repo):
            ...
            if "drift-pre-push" in str(p["path"]):
                console.print("  [dim]Activate with: git config core.hooksPath .githooks[/dim]")
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help=("Generate Claude Desktop MCP config snippet (claude_desktop_config.json)."),
)
@click.option(
    "--full",
    is_flag=True,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help=("Generate .github/copilot-instructions.md snippet for GitHub Copilot."),
)
@click.option(
    "--cursor",
    is_flag=True,
...
@click.option(
    ...
    default=False,
    help=(
        "Auto-detect profile from repo characteristics and write config (no prompts, CI-friendly)."
    ),
)
@click.option(
    "--repo",
...
def init(
    ...
    if not json_output and not dry_run:
        ...
        if _ascii_only:
            console.print(
                f"Setting up drift for {repo.name}\nProfile: {_prof.name} -- {_prof.description}"
            )
        else:
            console.print(
                Panel(
