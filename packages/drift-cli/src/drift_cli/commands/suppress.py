...
def list_suppressions(repo: Path, config: Path | None, check_stale: bool) -> None:
    ...
    if check_stale:
        stale_entries = [
            e for e in entries if e.stored_hash is not None and e.current_hash != e.stored_hash
        ]
...
def audit_suppressions(repo: Path, config: Path | None, today: datetime | None) -> None:
    ...
    files = _discover_files(repo, config)
    entries = collect_inline_suppressions(files, repo)
    expired = [entry for entry in entries if entry.until is not None and entry.until < current_day]
    ...
    console.print(table)
    raise click.ClickException(f"Found {len(expired)} expired inline suppression(s).")
...
def _display_finding(finding: Finding, idx: int, total: int, repo: Path) -> None:
    ...
    if finding.file_path:
        abs_path = (
            repo / finding.file_path if not finding.file_path.is_absolute() else finding.file_path
        )
...
def _apply_triage_suppress(
    ...
    until: date | None = date.today() + timedelta(days=temporal_days) if choice == "y" else None
