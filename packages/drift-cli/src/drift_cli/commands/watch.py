...
def _print_nudge_summary(result: dict, *, initial: bool = False) -> None:
    ...
    if new_findings:
        console.print(f"  [red]+{len(new_findings)} new[/] finding(s)", highlight=False)
    ...
    if resolved:
        console.print(f"  [green]-{len(resolved)} resolved[/] finding(s)", highlight=False)
...
def _build_nudge_summary(
    ...
    *,
    ...
    resolved_findings = [
        {"rule_id": f.get("rule_id", "")} for f in resolved_raw if isinstance(f, dict)
    ]
...
def _write_nudge_json(summary: dict[str, Any], output_path: Path) -> None:
    ...
    tmp_fd, tmp_name = tempfile.mkstemp(dir=output_path.parent, prefix=".nudge_", suffix=".tmp")
    try:
        import os
...
def _estimated_signal_labels(result: dict) -> list[str]:
    ...
    labels = {signal_abbrev(signal).upper() for signal in estimated_raw if signal}
