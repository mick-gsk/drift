...
"""Compat stub: delegates to drift_engine.self_improvement (ADR-102 Phase C)."""
...
# Redirect __path__ so submodule imports resolve to drift_engine.self_improvement.X
__path__ = _target.__path__  # type: ignore[assignment]


def __getattr__(name: str) -> object:
    """Lazy-register submodule on first access to avoid circular imports."""
    _src = "drift_engine.self_improvement." + name
    try:
