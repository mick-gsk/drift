...
class OrchestrationMetrics:  # drift:ignore[DCA]
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        discarded_ratio = (failed + expired) / claimed if claimed > 0 else 0.0
        plan_reuse = completed / max(self.plans_created, 1) if self.plans_created > 0 else 0.0
...
class DriftSession:  # drift:ignore[DCA]
    ...
    _seen_verification_payload_hashes: set[str] = field(default_factory=set, init=False, repr=False)
    ...
    def tasks_remaining(self) -> int:
        ...
        return sum(
            1 for t in self.selected_tasks if t.get("id", t.get("task_id", "")) not in excluded
        )
    ...
    def _effectiveness_warnings(self) -> list[dict[str, Any]]:
        ...
        resolved_per_file = (
            m.resolved_findings_total / m.changed_files_total if m.changed_files_total > 0 else 0.0
        )
        ...
        if (
            ...
            and (
                resolved_per_file < float(thresholds["low_effect_resolved_per_changed_file"])
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "ttl_remaining_seconds": max(0.0, self.ttl_seconds - (now - self.last_activity)),
    ...
    def end_summary(self) -> dict[str, Any]:
        ...
        if self.score_at_start is not None and self.last_scan_score is not None:
            ...
            result["score_delta"] = round(self.last_scan_score - self.score_at_start, 2)
    ...
    def _reap_expired_leases(self, max_reclaim: int = 3) -> None:
        ...
        now = time.time()
        expired_ids = [
            tid for tid, lease in self.active_leases.items() if lease["expires_at"] <= now
        ]
    ...
    def _emit_queue_event(self, event_type: str, payload: dict[str, Any]) -> None:
        ...
        ``drift_session.session_queue_log`` and ADR-081 for the persistence contract.
        """
        try:
            from drift_session.session_queue_log import QueueEvent, append_event
    ...
    def release_task(
        ...
        with self._lock:
            ...
            self.task_reclaim_counts[task_id] = self.task_reclaim_counts.get(task_id, 0) + 1
        ...
        with self._lock:
            ...
            if lease is None:
                task_exists = self.selected_tasks and any(
                    t.get("id", t.get("task_id", "")) == task_id for t in self.selected_tasks
                )
                if task_exists:
                    return {
                        "task_id": task_id,
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "seen_verification_payload_hashes": sorted(self._seen_verification_payload_hashes),
        }
...
class SessionManager:
    ...
    def save_to_disk(self, session_id: str, directory: str | Path | None = None) -> Path | None:
