...
from drift_engine.signals.base import create_signals

# Auto-discover all signal modules so @register_signal decorators execute.
for _finder, _mod_name, _ispkg in pkgutil.iter_modules(drift_engine.signals.__path__):
    importlib.import_module(f"drift_engine.signals.{_mod_name}")
...
def analyze_diff(
    ...
    from drift_engine.ingestion.git_history import _git_repo_prefix
