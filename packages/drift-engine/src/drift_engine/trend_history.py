...
from drift_sdk.models import RepoAnalysis, TrendContext
from drift_sdk.models._enums import TrendDirection

from drift_engine.scoring.engine import compute_signal_scores
...
def build_trend_context(current_score: float, snapshots: list[dict]) -> TrendContext:
    ...
    valid = [s for s in snapshots if isinstance(s.get("drift_score"), (int, float))]
