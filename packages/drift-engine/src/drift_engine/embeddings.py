...
class EmbeddingService:  # drift:ignore[DCA]
    ...
    def __init__(
        ...
        self._cache = EmbeddingCache(cache_dir, model_name=model_name) if cache_dir else None
...
def get_embedding_service(
    ...
    with _SERVICE_LOCK:
        ...
        if requested_cfg != _SERVICE_CFG:
            logger.warning(
                "Reinitializing embedding service due to changed configuration: old=%s new=%s",
                _SERVICE_CFG,
                requested_cfg,
            )
            _SERVICE = EmbeddingService(
