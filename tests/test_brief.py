...
# ---------------------------------------------------------------------------
# Issue #544 — Friction 1: deferred-file annotation in guardrail dicts
# ---------------------------------------------------------------------------


class TestGuardrailDeferredAnnotation:
    """_annotate_guardrails_deferred attaches deferred metadata to matching dicts."""

    def _make_gd(self, files: list[str]) -> dict:
        return {
            "id": "GR-PFS-001",
            "signal": "PFS",
            "constraint_class": "PATTERN",
    ...
    def _make_cfg(self, patterns: list[str]):
        from types import SimpleNamespace
        areas = [SimpleNamespace(pattern=p) for p in patterns]
        return SimpleNamespace(deferred=areas)

    def test_matching_file_gets_deferred_warning(self) -> None:
        from drift.api.brief import _annotate_guardrails_deferred

        gd = self._make_gd(["legacy/old.py", "src/active.py"])
        cfg = self._make_cfg(["legacy/**"])
        result = _annotate_guardrails_deferred([gd], cfg)
        assert len(result) == 1
        assert "deferred_files_involved" in result[0]
        assert "legacy/old.py" in result[0]["deferred_files_involved"]
        assert "deferred_warning" in result[0]

    def test_non_matching_file_unchanged(self) -> None:
        from drift.api.brief import _annotate_guardrails_deferred

        gd = self._make_gd(["src/active.py"])
        cfg = self._make_cfg(["legacy/**"])
        result = _annotate_guardrails_deferred([gd], cfg)
        assert "deferred_files_involved" not in result[0]
        assert "deferred_warning" not in result[0]

    def test_no_deferred_config_returns_unchanged(self) -> None:
        from types import SimpleNamespace

        from drift.api.brief import _annotate_guardrails_deferred

        gd = self._make_gd(["legacy/old.py"])
        cfg = SimpleNamespace(deferred=[])
        result = _annotate_guardrails_deferred([gd], cfg)
        assert "deferred_files_involved" not in result[0]

    def test_all_files_deferred_note_says_all(self) -> None:
        from drift.api.brief import _annotate_guardrails_deferred

        gd = self._make_gd(["legacy/a.py", "legacy/b.py"])
        cfg = self._make_cfg(["legacy/**"])
        result = _annotate_guardrails_deferred([gd], cfg)
        d = result[0]
        assert len(d["deferred_files_involved"]) == 2
        assert "2 of 2" in d["deferred_warning"]

    def test_dict_is_not_mutated_in_place(self) -> None:
        from drift.api.brief import _annotate_guardrails_deferred
        ...
        cfg = self._make_cfg(["legacy/**"])
        result = _annotate_guardrails_deferred([gd], cfg)
        # The original dict object must not be mutated
        assert "deferred_files_involved" not in gd
