...
# ---------------------------------------------------------------------------
# T011: calibrate explain — asserts all evidence fields visible (US3)
# ---------------------------------------------------------------------------


def test_explain_shows_tp_fp_fn_precision_confidence(tmp_path: Path) -> None:
    runner = CliRunner()
    cfg = _make_cfg(tmp_path)

    ev = MagicMock()
    ev.total_observations = 10
    ev.fn = 2
    ev.tp = 7
    ev.fp = 1
    ev.precision = 0.875
    ...
    build_result = MagicMock()
    build_result.total_events = 10
    build_result.evidence = {"pfs": ev}
    build_result.confidence_per_signal = {"pfs": 0.5}
...
# ---------------------------------------------------------------------------
# T013: calibrate status — asserts all required status fields visible (US4)
# ---------------------------------------------------------------------------


def test_status_shows_feedback_path_min_samples_auto_recalibrate(tmp_path: Path) -> None:
    runner = CliRunner()
    cfg = _make_cfg(tmp_path, enabled=True, min_samples=7, auto_recalibrate=True)
    ...
    assert result.exit_code == 0
    assert "Feedback events:" in result.output
    assert "Feedback path:" in result.output
    assert "Min samples" in result.output
    assert "Auto-recalibrate:" in result.output


# ---------------------------------------------------------------------------
# T019: calibrate run --format json --dry-run — output is parseable JSON (SC-005)
# ---------------------------------------------------------------------------


def test_run_json_format_with_dry_run_parseable(tmp_path: Path) -> None:
    import json as _json

    runner = CliRunner()
    cfg = _make_cfg(tmp_path)
    diff = {"pfs": {"default": 0.1, "calibrated": 0.12, "delta": 0.02, "confidence": 0.9}}
    build_result = _make_build_result(diff=diff)
    ...
    with (
        ...
        result = runner.invoke(
            calibrate, ["run", "--repo", str(tmp_path), "--format", "json", "--dry-run"]
        )
...
# ---------------------------------------------------------------------------
# T020: calibrate run --dry-run — drift.yaml stays untouched (SC-006)
# ---------------------------------------------------------------------------


def test_dry_run_does_not_modify_drift_yaml(tmp_path: Path) -> None:
    runner = CliRunner()
    config_file = tmp_path / "drift.yaml"
    ...
    cfg = _make_cfg(tmp_path)
    diff = {"pfs": {"default": 0.1, "calibrated": 0.15, "delta": 0.05, "confidence": 1.0}}
    build_result = _make_build_result(diff=diff)
