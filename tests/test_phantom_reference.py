...
class TestPhantomReferenceSignal:
    ...
    def test_internal_artifact_cache_avoids_second_ast_parse(
        self,
        tmp_path: Path,
        monkeypatch,
    ):
        """Second run with unchanged files should not reparse AST in PHR."""
        files = {
            "pkg/__init__.py": "",
            "pkg/helpers.py": """\
                def clean(value):
        ...
        import drift.signals.phantom_reference as phr_module

        original_build = phr_module.PhantomReferenceSignal._build_file_artifact
        build_calls = 0

        def _counting_build(self, file_path):
            nonlocal build_calls
            build_calls += 1
            return original_build(self, file_path)

        monkeypatch.setattr(
            phr_module.PhantomReferenceSignal,
            "_build_file_artifact",
            _counting_build,
        )

        first_findings = _run_phr(files, tmp_path)
        assert build_calls > 0

        build_calls = 0
        second_findings = _run_phr(files, tmp_path)
        assert build_calls == 0

        assert [f.title for f in second_findings] == [f.title for f in first_findings]

    def test_internal_artifact_cache_reparses_only_changed_file(
        self,
        tmp_path: Path,
        monkeypatch,
    ):
        """When one file changes, only that file should be reparsed by PHR."""
        files_v1 = {
            "pkg/__init__.py": "",
            "pkg/helpers.py": """\
                def clean(value):
        ...
        files_v2 = {
            ...
            "pkg/main.py": """\
                ...
                def run(value):
                    return sanitize_value(clean(value))
            """,
        }
        ...
        _run_phr(files_v1, tmp_path)

        build_calls = 0
        findings = _run_phr(files_v2, tmp_path)
        assert build_calls == 1

        phr = [f for f in findings if f.signal_type == SignalType.PHANTOM_REFERENCE]
        assert len(phr) >= 1
        phantom_names = [p["name"] for p in phr[0].metadata["phantom_names"]]
        assert "sanitize_value" in phantom_names
