...
def test_generated_header_file_without_generated_suffix_is_ignored_for_avs_findings(
    tmp_path: Path,
):
    generated_file = tmp_path / "src" / "db" / "schema_base.ts"
    ...
    api_file = tmp_path / "src" / "api" / "routes.ts"
    api_file.parent.mkdir(parents=True, exist_ok=True)
    api_file.write_text("export const routes = {};\n", encoding="utf-8")
