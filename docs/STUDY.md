...
> **Feature update (2026-05-02, v2.50.0: VSA Migration Phase 1 — drift-verify package):** Introduces `packages/drift-verify/` as a new workspace package (`drift_verify`) providing evidence-based drift verification. Phase 1 delivers: `drift evidence` CLI subcommand (re-export stub via ADR-100 pattern), XDG config support (`test_xdg.py`), and 454-line unit test suite under `tests/verify/`. CI workflow updated to install `drift-verify` in all 3 pip-install blocks. `drift-cli/pyproject.toml` declares `drift-verify` as a workspace dependency. Non-goals: no new drift signals, no scoring weight changes. Tests: `tests/verify/` (5 files, ~693 lines). Evidence: `benchmark_results/v2.50.0_vsa-migration-phase1_feature_evidence.json`.

> **Feature update (2026-05-01, v2.50.0: Outcome-First Validation Study Runner):** Adds `scripts/outcome_first_validation.py` — a standalone 14-day outcome-first study runner implementing the metric contract from `docs/concepts/outcome-first-validation.md`. The runner defines `TaskManifestEntry` + `TaskRecord` frozen dataclasses, `CohortMetrics` aggregation, and a deterministic Go/No-Go evaluator against four thresholds (steps ≥25%, rework ≥30%, first-pass ≥15%, defect guardrail). New `session_summary_to_task_record()` derives a `TaskRecord` directly from a `DriftSession.summary()` dict (tool_calls → steps, plans_invalidated + nudge_degrading → rework_events). New `ingest-session` CLI subcommand writes a session-derived record to the rolling records file. Fixtures in `work_artifacts/outcome_first_validation/`. Non-goals: no new drift signals, no scoring weight changes. Tests: 16 passing in `tests/test_outcome_first_validation.py`. Evidence: `benchmark_results/v2.49.0_outcome-first-validation_feature_evidence.json`.

> **Feature update (2026-04-30, v2.48.5: drift pr-loop — Agent-driven PR Review Loop):** Adds `drift pr-loop <PR_NUMBER>` CLI command that drives a GitHub PR through an automated agent review loop (FR-001–FR-013). New vertical slice `src/drift/pr_loop/` with frozen Pydantic models (`_models.py`), gh CLI side-effects (`_gh.py`), pure state machine functions (`_engine.py`), state persistence (`_state.py`), Rich+JSON output (`_output.py`), and Click subcommand (`_cmd.py`). Config: `pr_loop:` section in `drift.yaml` / `PrLoopConfig` in `src/drift/config/_loader.py`. Exit codes: APPROVED=0, ESCALATED=1, ERROR=2, precondition=3. Loop persists state to `work_artifacts/pr-loop-<PR>.json`. Supports `--dry-run` (no GitHub side-effects), `--format json`, `--exit-zero`. Tests: `tests/pr_loop/` (73 passing). Non-goals: no new drift signals, no scoring weight changes. Evidence: `benchmark_results/v2.48.5_agent-pr-review-loop_feature_evidence.json`.

> **Feature update (2026-04-29, v2.48.3: Quickscan Skill + output_root config):** Adds `.github/skills/drift-repo-quickscan/SKILL.md` — a distributable one-shot repo health-check skill that guides any user through a 5-minute architectural analysis (install check, `drift analyze`, top-3 finding explanation, optional `drift init --auto`) without prior drift expertise. Adds `output_root` field to `DriftConfig` with `resolve_artifact_path()` helper: when set (or via `DRIFT_OUTPUT_ROOT` env var), all artifact paths (`.drift-cache`, `.drift/feedback.jsonl`, `.drift/history`, etc.) are resolved under the specified directory instead of the analysed repo root, keeping analysed repos clean. `drift.example.yaml` and `drift.schema.json` updated. `commands/_last_scan.py` and `commands/calibrate.py` updated to use `resolve_artifact_path`. New `tests/test_config_output_root.py` with 97 lines of coverage. Non-goals: no new signals, no scoring weight changes. Evidence: `benchmark_results/v2.48.3_quickscan-skill-output-root_feature_evidence.json`.
...
> **Feature update (2026-04-27, v2.47.1):** drift-kit multi-agent support, `--and-analyze` flag, and `/drift-feature-guardrails` prompt. **Multi-agent config (`--agent`):** `drift kit init` now accepts `--agent [cursor|claude|codex|all]` to generate agent-specific config files: `.cursor/rules/drift.mdc` for Cursor, appends a `<!-- drift-kit -->` section to `CLAUDE.md` for Claude Code, and to `AGENTS.md` for OpenAI Codex — idempotent on repeat runs. **`--and-analyze` flag:** runs `drift analyze` immediately after scaffolding so the session file is available for all prompt templates without a manual step. **`/drift-feature-guardrails` prompt:** new VS Code Copilot Chat slash command that reads `.vscode/drift-session.json` and maps a feature description to relevant drift signals (PFS/AVS/MDS/EDS/DCA), generating a risk table, guardrails, and clarifying questions before coding starts. **Improved missing-session guidance** in all three existing prompt templates: exact `drift analyze` command + MCP fallback (`drift_scan` tool). Fix: `record_feedback()` now supports `max_feedback_events` FIFO cap (FR-011b). Fix: `subprocess.run()` in `kit_cmd.py` now sets `stdin=DEVNULL` (MCP stdio safety). Evidence: `benchmark_results/v2.47.1_drift-kit-multi-agent_feature_evidence.json`.
