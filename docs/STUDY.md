...
> **Feature update (v2.42.10):** PFS, MDS, and EDS finding messages enriched with boundary vocabulary and change-cost language. **Description strings** now name the violated responsibility boundary or shared concern (Family A: `boundary`, `concern`, `responsibility`, `shared concern`). **Fix strings** now name the concrete change category that becomes expensive if the finding is unresolved (Family B: `change propagation`, `change effort`, `coupled`, `cost of change isolation`). **Skill update:** `drift-finding-message-authoring/SKILL.md` now includes a Boundary Vocabulary section (Family A + Family B) as the authoritative term reference for reason/next_action strings in all signals. **New test:** `tests/test_finding_message_quality.py` (8 keyword smoke-check tests, all green). Evidence: `benchmark_results/v2.42.10_findings-change-effort-coupling_feature_evidence.json`.
