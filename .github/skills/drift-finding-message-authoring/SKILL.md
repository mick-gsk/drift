...
Finding strings in mandatory signals (PFS, MDS, EDS) must contain at least one term
from **Family A** (responsibility language) or **Family B** (change-cost language).
This contract is tested in `tests/test_finding_message_quality.py`.

### Family A — Responsibility Language

Use when naming *where* the concern lives or *who* owns it:

| Term | Example use |
|---|---|
| `layer` | "crosses a layer boundary" |
| `boundary` | "missing shared boundary" |
| `service` | "different service namespaces" |
| `interface` | "protocol interface repeated across modules" |
| `concern` | "shared concern without a dominant convention" |
| `responsibility` | "missing shared responsibility boundary" |
| `domain` | "duplicated across domain modules" |
| `ownership` | "unclear ownership boundary" |
| `contract` | "violates the module contract" |

### Family B — Change-Cost Language

Use when explaining *why the finding is expensive to ignore*:

| Term | Example use |
|---|---|
| `effort` | "Effort: M — changes span two modules" |
| `change propagation` | "eliminates change propagation across N sites" |
| `coupled` | "coupled copies indicate a shared responsibility" |
| `change risk` | "increases change risk in downstream callers" |
| `isolat` | "isolating the concern reduces blast radius" |
| `expensive` | "expensive to change consistently" |
| `spread` | "spreads change effort across the module" |
| `ripple` | "ripple effect on callers" |
| `entangled` | "entangled responsibilities" |

### Rule

A finding **passes** when its `description` or `fix` contains at least one term from
Family A **or** Family B (case-insensitive substring match).

A finding **fails** when both fields are pattern-only (describe observed structure
without naming a responsibility boundary or change-cost implication).

Cross-workspace and plugin-isolation findings are **exempt** — their messages
describe intentional isolation and must not add generic boundary framing.
